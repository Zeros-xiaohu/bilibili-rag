...
| 场景          | 痛点                                                 | BiliMind 解决方案                        |
| :------------ | :--------------------------------------------------- | :--------------------------------------- |
| 学习复习      | 收藏几十个教程视频，想找一个知识点不知道在哪个视频里 | 自然语言搜索，直接定位到 XX 视频 XX 片段 |
| 技术调研      | 不同 UP 主对同一话题观点不同，想横向对比             | AI 综合多个视频回答，附带来源对比        |
| 播客/访谈归档 | 2 小时访谈，想回顾某个具体话题                       | ASR 全文转写后语义检索，无需拖进度条     |
| 碎片化整理    | 收藏夹里几百个视频从不分类                           | AI 自动按内容主题归档                    |

用户路径设计

```
                   ┌─ 不登录 ──────→ 浏览首页介绍 + 体验 Demo 流程
                   │
用户访问首页 ──────┤
                   │                    ┌─ 选收藏夹 → 点"入/更新库" → 等待同步
                   └─ 扫码登录 ──────→ │
